...
<script>
    layui.config({
        ...
        var tableIns= table.render({
            ...
            ,url: '${pageContext.request.contextPath}/topics/list'
            ...
            ,cols: [[
                ...
                ,{field:'username', title:'发布用户', align:'center', width:120,templet:function (res) {
                    if(res.account != null &&  res.account.username != null) {
                        return res.account.username
                    }
                        return ''
                }}
                ,{field:'comtent', align:'center', title:'留言内容'}
                ,{field:'timeStr', align:'center', title:'发布时间', width:180,sort: true}
                ,{field:'typename', align:'center', title:'留言主题 ', width:180,templet:function (res) {
                     if(res.typename != null){
                         return  `<span style="margin-right: 2px;" onclick="queryLable('`+res.typename+`')" class="layui-badge layui-bg-green">`+res.typename+'</span>'
                     }
                     return '';
                }}
        ...
        $('#queryRole').on('click', function(event,froms){
            ...
            tableIns.reload({
                url: '${pageContext.request.contextPath}/topics/list?content='+queryContent
                ,page: {
                    curr:1
                }
            })
                ...
                case 'getCheckData':
                    ...
                    if( ids=="" || ids==null){
                        ...
                        layer.confirm('确定删除'+ids.length+'数据', function(index){
                            ...
                            $.post("${pageContext.request.contextPath}/topics/deleteIds",{ids}, (res) => {
                                var  queryContent = $('#queryContent').val()
                                // tableIns.reload({
                                //     url: 'http://localhost:8080/resources/list?content='+queryContent
                                // })
        ...
        table.on('tool(test-table-toolbar)', function(obj){
            ...
            if(obj.event === 'del'){
                layer.confirm('确定删除', function(index){
                    obj.del();
                    $.get("${pageContext.request.contextPath}/topics/delete?id="+data.id);
                    layer.msg('已删除!',{icon:1,time:1000});
                    layer.close(index);
                });
            }
