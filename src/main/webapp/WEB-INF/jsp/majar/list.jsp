                ...
                <div class="layui-card-body">
                    ...
                    <div class="layui-row" align="center" style="height: 40px">
                            ...
                            <form class="layui-form layui-col-md12 x-so" method="get" action="#">
                                <input type="text" id="queryContent" name="content" style="width:50%;"  placeholder="请输入学院或专业" autocomplete="off" class="layui-input" >
                    ...
                    <script type="text/html" id="test-table-toolbar-barDemo">
                        ...
                        <a class="layui-btn layui-bg-blue layui-btn-xs" lay-event="manage">班级管理</a>
                    </script>
                </div>
...
<script>
    layui.config({
        ...
        var tableIns= table.render({
            ...
            ,url: '${pageContext.request.contextPath}/majar/list'
            ,toolbar: 'default'
            ,cellMinWidth: 60
            ,height : 'full-90'
            ,title: '资源数据表'
            ,cols: [[
                 {type: 'checkbox'}
                ,{field:'id', title:'ID',align:'center',width: 60 , unresize: true, sort: true}
                ,{field:'institute', align:'center',title:'学院',width: 220}
                ,{field:'majarname', align:'center',title:'专业',width: 230}
                ,{field:'typelist', title:'班级',templet: function(res){
                        var  classList =  res.classList
                        if(classList.length >5){
                            return `<span style="margin-right: 2px;"  class="layui-badge layui-bg-green">本专业共有`+classList.length+'个班级</span>';
                        }
                        var node = ''
                        classList.forEach(function(e){
                            node = node + `<span style="margin-right: 2px;"  class="layui-badge layui-bg-green">`+e.classnum+'</span>'
                        });
                        return node;
                    }}
                ,{field:'institnum',align:'center', title:'学院代码',width: 80}
            ...
            switch(obj.event){
                case 'delete':
                    ...
                    if( ids=="" || ids==null){
                        ...
                        layer.confirm('确定删除'+ids.length+'数据', function(index){
                            ...
                            $.post("${pageContext.request.contextPath}/majar/deleteIds",{ids}, (res) => {
                                var  queryContent = $('#queryContent').val()
                                tableIns.reload()
                ...
                case 'add':
                    var index = layer.open({
                        ...
                        content: ["${pageContext.request.contextPath}/majar/toadd", "no"],
                        end: function() {
                            //tableIns.reload({})
                        },
                ...
                case 'update':
                    ...
                    if(data.length === 0){
                        ...
                        var index = layer.open({
                            ...
                            shade: 0.4,
                            skin: 'layui-layer-rim',
                            content: ["${pageContext.request.contextPath}/majar/toedit?id="+data[0].id, "no"],
                            end: function() {
                                //tableIns.reload({})
                            },
        ...
        table.on('tool(test-table-toolbar)', function(obj){
            ...
            if(obj.event === 'del'){
                layer.confirm('确定删除', function(index){
                    obj.del();
                    $.get("${pageContext.request.contextPath}/majar/delete?id="+data.id);
                    layer.msg('已删除!',{icon:1,time:1000});
                    layer.close(index);
                });
                ...
                var index = layer.open({
                    ...
                    shade: 0.4,
                    skin: 'layui-layer-rim',
                    content: ["${pageContext.request.contextPath}/majar/toedit?id="+data.id, "no"],
                    end: function() {
                        //tableIns.reload({})
                    },
                ...
                layer.open({
                    ...
                    ,title: '班级管理'
                    ,shadeClose: true
                    ,content: '${pageContext.request.contextPath}/adminclass/toclassList?id='+data.id
                    ,maxmin: true
                    ,area: ['100%', '100%']
