            ...
            <c:forEach items="${list}" var="info">
                <div id="${info.id}" class="media-body" style="margin-left: 20px;margin-bottom: 25px">
                    <a href="javascript:;" class="media-left" style="float: left;">
                        <img src="${pageContext.request.contextPath}${info.account.pic}" height="46px" width="46px">
                    </a>
                    <div class="pad-btm">
                    ...
                    <c:if test="${info.conList.size() != 0}">
                        <div style="margin-top: 20px;margin-left: 20px" class="layui-timeline-content layui-text">
                            ...
                            <span class="layui-breadcrumb" lay-separator="">
                                <span>
                                    <a  href="javascript:;"style="text-decoration-line: none;font-size: 15px;">${comment.fromUsername}</a>
                                    <span style="font-size: 10px;color: orange">${comment.toUid == info.account.id? "评论":"回复"}</span>
                                    <a  href="javascript:;"style="text-decoration-line: none;font-size: 15px;">${comment.toUid == info.account.id? "":comment.toUsername} ：</a>
...
<script>
    ...
    layui.config({
        ...
        laypage.render({
            ...
            ,curr:${pageNo == null ? 1:pageNo}
            ,limit:${pageSize == null ? 10:pageSize}
            ,first:1
            ,last:${pages == null ? 0:pages}
            ...
            ,jump: function(obj, first){
                ...
                if(!first){
                    ...
                    if(content != null && content != ''){
                        location.href ="${pageContext.request.contextPath}/front/frontTopics?page="+obj.curr+"&limit="+obj.limit+"&content="+content
                    }else {
                        location.href="${pageContext.request.contextPath}/front/frontTopics?page="+obj.curr+"&limit="+obj.limit
                    }
        ...
        form.on('submit(formDemo)', function(data){
            ...
            $.post("${pageContext.request.contextPath}/topics/add",topics,function (data) {
                ...
                layer.msg("已发布")
                setTimeout(function () {
                    location.href="${pageContext.request.contextPath}/front/frontTopics"
                },500)
            })

            return false;
        });
        layui.layer.msg('前台留言仅供浏览', {
            offset: '100px'
            ,time: 3000
        });
    });
    ...
    function querys(content) {
        if(content){
            location.href="${pageContext.request.contextPath}/front/frontTopics?page=1&limit="+'${pageSize}'+"&content="+content
        }else {
            location.href="${pageContext.request.contextPath}/front/frontTopics?page=1&limit="+'${pageSize}'
        }
    }
